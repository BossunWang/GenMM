...
from utils.rotation_conversions import quaternion_apply
...
JOINT_NAMES_HYBRIK_MIRROR = [
        'pelvis', 'right_hip', 'left_hip',      # 2
        'spine1', 'right_knee', 'left_knee',    # 5
        'spine2', 'right_ankle', 'left_ankle',  # 8
        'spine3', 'right_foot', 'left_foot',    # 11
        'neck', 'right_collar', 'left_collar',  # 14
        'head',                                 # 15
        'right_shoulder', 'left_shoulder',      # 17
        'right_elbow', 'left_elbow',            # 19
        'right_wrist', 'left_wrist',          # 21
        'jaw',                                  # 22
        'right_eye_smplhf', 'left_eye_smplhf',  # 24
        # hand
        'right_index1', 'right_index2', 'right_index3',     # 27
        'right_middle1', 'right_middle2', 'right_middle3',  # 30
        'right_pinky1', 'right_pinky2', 'right_pinky3',     # 33
        'right_ring1', 'right_ring2', 'right_ring3',        # 36
        'right_thumb1', 'right_thumb2', 'right_thumb3',     # 39
        'left_index1', 'left_index2', 'left_index3',        # 42
        'left_middle1', 'left_middle2', 'left_middle3',     # 45
        'left_pinky1', 'left_pinky2', 'left_pinky3',        # 48
        'left_ring1', 'left_ring2', 'left_ring3',           # 51
        'left_thumb1', 'left_thumb2', 'left_thumb3',        # 54
        # leaf joints
        'RBigToe', 'LBigToe',                   # 56
        'mouth_bottom',                                 # 57
        'reye', 'leye',                         # 59
        'rindex', 'rmiddle', 'rpinky', 'rring', 'rthumb',   # 64
        'lindex', 'lmiddle', 'lpinky', 'lring', 'lthumb',
        'mouth_top',                            # 70
]
...
def plot_single_pose(num, poses, lines, ax, axrange, scat, contact, parent, zcenter, show_txt=False):
    pose = poses[num]
    static = contact[num]
    indices = [7, 8, 10, 11]
    ...
    for i, (p, line) in enumerate(zip(parent, lines)):
        # don't plot root
        if i == 0:
            continue
        ...
        if show_txt:
            label = '%d,%d' % (i, p)
            ax.text(pose[i, 0] + 0.0, pose[i, 1] + 0.0, pose[i, 2] + 0.0, label, size=10, zorder=1)
...
def skeleton_render(
    ...
    point = np.array([0, 0, np.min(poses[0, :, 2])])
    ...
    lines = [
        ...
        for _ in parents[:poses.shape[1]]
    ]
    scat = [
    ...
    if render:
        ...
        anim = animation.FuncAnimation(
            ...
            plot_single_pose,
            num_steps,
            fargs=(poses, lines, ax, axrange, scat, contact, parents, zcenter),
            interval=1000 // FPS,
        )
    if sound:
        ...
        if render:
            out = os.system(
                f"ffmpeg -loglevel error -y -r {FPS} -i {gifname} -i {audioname} -shortest -c:v libx264 -c:a aac -q:a 4 {outname}"
            )
    else:
        if render:
            # actually save the gif
            gifname = os.path.join(out, f"{name}.gif")
            ...
            out = os.system(
                f"ffmpeg -loglevel error -y -r {FPS} -i {gifname} -shortest -c:v libx264 -c:a aac -q:a 4 {outname}"
            )
        else:
            # debug
            plot_single_pose(0, poses, lines, ax, axrange, scat, contact, parents, zcenter, show_txt=True)
            plt.show()
    plt.close()
...
class SMPLSkeleton:
    ...
    def forward(self, rotations, root_positions):
        ...
        # print("rotations: ", rotations[0, 1, 1, 1], rotations[0, 1, 12, 1], rotations[0, 1, 22, 1])
        ...
        # print("expanded_offsets: ", expanded_offsets[0, 1, 1, 1], expanded_offsets[0, 1, 12, 1], expanded_offsets[0, 1, 22, 1])
        ...
        for i in range(self._offsets.shape[0]):
            if self._parents[i] == -1:
                ...
                # print("positions_world:", positions_world[self._parents[i]][0, 0])
                # print("rotations q:", rotations_world[self._parents[i]][0, 0])
                # print("expanded_offset:", expanded_offsets[0, 0, i])
                # print("quaternion_apply:", quaternion_apply(
                #         rotations_world[self._parents[i]], expanded_offsets[:, :, i]
                #     )[0, 0])
                # print("new positions_world:", positions_world[-1][0, 0])

                if self._has_children[i]:
                    # print("rotations_world q:", rotations_world[self._parents[i]][0, 0])
                    # print("rotations q:", rotations[:, 0, i])
        ...
        # print("rotations_world[0] shape:", rotations_world[0].shape)
        # print("rotations_world[0]: ", rotations_world[0][0, 1, 1], rotations_world[0][0, 40, 1], rotations_world[0][0, 400, 1])
        # print("rotations_world[1]: ", rotations_world[1][0, 1, 1], rotations_world[1][0, 40, 1], rotations_world[0][0, 400, 1])
...
class SMPLXSkeleton:
    def __init__(
        self, offsets, parents, device=None,
    ):
        assert len(offsets) == len(parents)
    ...
    def forward(self, rotations, root_positions):
        ...
        Perform forward kinematics using the given trajectory and local rotations.
        Arguments (where N = batch size, L = sequence length, J = number of joints):
         -- rotations: (N, L, J, 4) tensor of axis-angle rotations describing the local rotations of each joint.
         -- root_positions: (N, L, 3) tensor describing the root joint positions.
        """
        assert len(rotations.shape) == 4
        assert len(root_positions.shape) == 3
        ...
        for i in range(self._offsets.shape[0]):
            if self._parents[i] == -1:
                ...
                if self._has_children[i]:
                    # using standardize_quaternion from qw = cos(angle/2)
                    # range should be 0~1
                    rotations_world.append(
                        quaternion_multiply(
                            rotations_world[self._parents[i]], rotations[:, :, i]
                    ...
                    # print("rotations_world q:", rotations_world[self._parents[i]][0, 0])
                    # print("rotations q:", rotations[:, 0, i])
                    # print("new rotations world:", quaternion_multiply(
                    #         rotations_world[self._parents[i]], rotations[:, :, i]
                    #     )[0, 0])
                else:
                    # This joint is a terminal node -> it would be useless to compute the transformation
